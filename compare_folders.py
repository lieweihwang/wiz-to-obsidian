...
with open('compare_result.txt', 'w', encoding='utf-8') as f_out:
    f_out.write(f"WizNote 源目录 (.ziw) 文件数量: {len(ziw_files)}\n")
    f_out.write(f"Obsidian 输出目录 (.md) 文件数量: {len(md_files)}\n")

    # 比较每个目录下的文件数量
    # 注意，Windows 路径在大小写方面是不敏感的，但在字典中区分大小写。
    # 为了避免因为 'Python' 和 'python' 造成的差异，统一转成小写比较
    wiz_dirs = {}
    for z in ziw_files:
    ...
    # 检查是否有 .md 文件但没有对应的源目录
    extra_in_obs = []
    for d, obs_count in obs_dirs.items():
        if d not in wiz_dirs:
            extra_in_obs.append((d, obs_count))

    f_out.write(f"\n输出目录中有，但源目录中没有对应目录的数: {len(extra_in_obs)}\n")
    if extra_in_obs:
        f_out.write("示例:\n")
        for d, c in extra_in_obs[:10]:
             f_out.write(f"  - {d} (包含 {c} 个 .md)\n")
