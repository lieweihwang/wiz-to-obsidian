...
class NoteSynchronizer:
    ...
    def _download_img_if_absent(self, record, need_upload_images, cached_detail=None):
        # 获取本地不存在的图片文件
        ...
        if not Note.is_collaboration_note(record['type']):
            # 如果是普通笔记, 复用已缓存的 detail（避免重复请求）
            detail_resp = cached_detail if cached_detail is not None else self.api_client.get_note_detail(record['doc_guid'])
            resources = detail_resp.get('resources', [])
    ...
    def _save_img_and_get_url(self, record: dict, need_upload_images: list[str], cached_detail=None) -> dict[str, str]:
        """
        保存并上传图片, 获取没有上传图片的<图片名称, 上传地址>
        :param record: 笔记记录
        :param need_upload_images: 需要上传的图片集合
        :param cached_detail: 已缓存的 get_note_detail 响应，避免重复请求
        ...
        ret_map = {}
        # 如果图片不存在则下载图片（传入缓存的 detail 避免重复请求）
        self._download_img_if_absent(record, need_upload_images, cached_detail=cached_detail)
        for img_file_name in need_upload_images:
            # 插入上传记录
    ...
    def _sync_single_note_to_local(self, record):
        ...
        try:
            # 获取笔记原始内容（普通笔记同时缓存 detail，避免后续重复请求）
            cached_detail = None
            if Note.is_collaboration_note(record['type']):
                origin_content = self._get_note_origin_content(record['type'], record['doc_guid'])
            else:
                cached_detail = self.api_client.get_note_detail(record['doc_guid'])
                origin_content = cached_detail['html']

            # 使用解析器解析笔记，将笔记转化为md, 并提取笔记中需要上传的图片
            parsed_note = parser.process_content(origin_content)
            # 上传并获取上传图片地址（传入缓存 detail，避免重复网络请求）
            origin_img_image_url_map = self._save_img_and_get_url(
                record, parsed_note.need_upload_images, cached_detail=cached_detail
            )
    ...
    def _process_collaboration_note_attachments(self, record, parsed_note: ParsedNote):
        ...
        for attachment_name in attachment_links:
            ...
            try:
                ...
                relative_path = f"./{record['title']}/{attachment_name}"
    ...
    def _process_normal_note_attachments(self, record, parsed_note: ParsedNote):
        ...
        :param parsed_note: 解析后的笔记对象
        """
        # 如果 attachment_count 为 0，跳过网络请求（由 insert_note_list 从 API 响应写入）
        attachment_count = record.get('attachment_count', -1)
        if attachment_count == 0:
            log.info(f'跳过附件查询（attachment_count=0）: {record["doc_guid"]}')
            return None
        ...
        for attachment in attachments:
            ...
            try:
                ...
                relative_path = f"./{record['title']}/{att_name}"
