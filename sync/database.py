...
class Database:
    ...
    def init(self):
        ...
        cursor.execute('''
            CREATE TABLE IF NOT EXISTS note_sync_rec (
                ...
                attachment_count INTEGER DEFAULT -1, -- 附件数量，-1表示未知，0表示无附件
                create_time TIMESTAMP default (datetime('now', 'localtime')),
                update_time TIMESTAMP default (datetime('now', 'localtime'))
            )
        ''')

        # 已有数据库的迁移：添加 attachment_count 列（若不存在）
        try:
            cursor.execute("ALTER TABLE note_sync_rec ADD COLUMN attachment_count INTEGER DEFAULT -1")
            self.conn.commit()
            log.info("已迁移 note_sync_rec: 添加 attachment_count 列")
        except Exception:
            pass  # 列已存在，忽略
    ...
    def insert_note_list(self, note_list):
        ...
        try:
            cursor.executemany('''
                INSERT OR IGNORE INTO note_sync_rec (doc_guid, type, title, category, created, accessed, url, sync_status, attachment_count)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', data)
            self.conn.commit()
        except Exception as e:
    ...
    def get_unsync_note_list(self, max_version, page_size):
        rows = self.query(f'''
            SELECT id, doc_guid, type, title, category, created, accessed, url, attachment_count FROM note_sync_rec
