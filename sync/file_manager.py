...
class FileManager:
    ...
    def get_img_directory(record):
        """
        获取图片保存目录: 当前笔记同级目录下，以笔记标题命名的子文件夹。
        匹配 Obsidian 附件路径配置: ${notepath}/${notename}
        :param record: 笔记同步记录
        :return:
        """
        app_root = FileManager.get_app_root()
        title = FileManager.sanitize_filename(record.get('title', 'images'))
        return os.path.join(app_root, "output", "note", record['category'].strip("/").replace("/", os.path.sep), title)

    @staticmethod
    def get_attachments_directory(record):
        """
        获取附件保存目录: 当前笔记同级目录下，以笔记标题命名的子文件夹。
        匹配 Obsidian 附件路径配置: ${notepath}/${notename}
        :param record: 笔记同步记录（需要 category 和 title 字段）
        :return: 附件目录完整路径
        """
        app_root = FileManager.get_app_root()
        title = FileManager.sanitize_filename(record.get('title', 'attachments'))
        return os.path.join(app_root, "output", "note", record['category'].strip("/").replace("/", os.path.sep), title)
